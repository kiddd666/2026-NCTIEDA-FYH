...
- 外部依赖：Public case、正式 Liberty、参考提交和环境说明已入库；工具手册正文、License 与真实运行报告仍待验证。未获真实证据时，不把公开样例或推测写成 Hidden case 的工具事实，详见[[05-待确认事项与来源索引#4.3 2026-09 官方材料确认范围|官方材料确认范围]]。
...
- 广立微工具手册：环境说明已列镜像内路径；实际规则编号、判定条件和建议修复仍待读取与运行校准；
- Public case：已提供 `preset_issues.json`、原始 Dofile 和正确 Dofile，可作为实训主材料；真实日志和 DRC 报告仍待运行产生。
...
| 第 15 日 | Public case 静态校准与工具手册实训；工具未运行时仅建立来源清晰的矩阵 | 第一份需求—配置—证据矩阵 | 每条要求都有配置对象和验收报告，不以退出码代替成功 |

**第三周关口**：能从任务说明拆出 DFT Signal、Internal Scan、Lockup、Wrapper 和输出要求，并为每项指定报告证据。当前可用 Public 材料校准静态结构；未完成真实运行前，不验收命令语义或报告字段事实。
...
| 第 19 日 | 任务二问题闭环：现象—对象—根因—修复—验证 | 至少 6 个问题的诊断记录 | 修复落在根因，不用未经任务授权的 Ignore、降级或屏蔽报告替代修复 |
