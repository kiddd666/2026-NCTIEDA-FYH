...
| ADR-007 | 2026-08-30 | 以 TODO 为待办事实源并用独立看板展示 | 已接受 |
...
## ADR-009 — 采用 Kanban Markdown + Codex skill 作为个人看板工作流
...
FYH 已有 `TODO.md` 和 `KANBAN.md` 的文字看板约定，但缺少可拖拽、可持续同步
的 VS Code 入口。用户已安装 Kanban Markdown 扩展，并要求在个人仓库中配置使用。

### 决策

采用 Kanban Markdown 扩展渲染 Markdown 卡片，并安装 `kanban-markdown` Codex
skill 管理卡片。看板数据放在团队工作区下的个人目录 `FYH/.devtool/features/`；工作区设置
默认使用 Codex 的 AI 操作入口。`TODO.md` 继续作为任务语义和验收事实源，看板
负责状态可视化与卡片编辑。

### 备选方案

- 继续只维护 `KANBAN.md`：无需扩展，但缺乏拖拽和卡片元数据自动维护。
- 将看板放在团队仓库根目录：会把个人卡片混入团队工作区。
- 只使用扩展、不安装 skill：人工编辑可行，但 Codex 无法稳定遵守卡片 Schema。

### 理由

Markdown 文件保持可 diff、可回溯和离线可用；FYH 独立仓库隔离个人状态；扩展
提供交互界面，skill 提供可验证的 front matter 和状态迁移规则，两者职责互补。

### 影响

- VS Code 应以 `D:\research\2026-NCTIEDA-Semitronix` 为工作区根目录。
- 新卡片使用 `.devtool/features/{id}.md`，Done 卡片位于 `done/` 子目录。
- 迁移状态时必须维护 `status`、`order`、`modified` 和 `completedAt` 的一致性。
- 看板状态变化不自动触发 `progress.md`，仍按阶段性状态标准单独判断。
...
- 本机 `kanban-markdown` Codex skill

## ADR-010 — 分离团队工作区与 FYH 个人看板数据
...
VS Code 实际打开的是团队仓库根目录，而 FYH 只是其中的个人文件夹。若把工作区
配置放在 `FYH/.vscode`，团队根工作区不会读取它；若把卡片放在根目录，又会把
个人看板混入团队项目。

### 决策

将 Kanban 工作区配置放在团队根 `.vscode/settings.json`，将
`kanban-markdown.featuresDirectory` 指向 `FYH/.devtool/features`。该设置通过
本地 `.git/info/exclude` 忽略，不进入团队提交；FYH 个人仓库继续拥有卡片数据。

### 备选方案

- 继续使用 `FYH/.vscode`：当前工作区根目录下不会生效。
- 将卡片放在团队根 `.devtool/features`：破坏个人与团队数据边界。
- 修改团队共享 `.gitignore`：会把个人配置策略写入团队规则。

### 理由

配置必须位于实际工作区根才能稳定生效，卡片数据则应留在个人目录；本地 exclude
同时避免个人设置污染团队 Git 历史。

### 影响

- 打开看板时必须以 `D:\research\2026-NCTIEDA-Semitronix` 为工作区根目录。
- 个人卡片统一位于 `FYH/.devtool/features/`。
- 团队成员不会因个人看板设置而改变其工作区配置。
