# FYH 技术决策记录

> FYH 在 Scan Insertion 项目中的长期技术与工作流决策记录。
> 普通 bugfix、格式修改和临时调试不写入本文件；原则上只追加。

## 决策索引

| 编号 | 日期 | 决策 | 状态 |
| --- | --- | --- | --- |
| ADR-001 | 2026-08-30 | 统一个人记录位置并使用仓库级本地覆盖规则 | 已接受 |
| ADR-002 | 2026-08-30 | 个人进度与决策日志统一使用中文 | 已接受 |
| ADR-003 | 2026-08-30 | 将 progress 从操作流水账收敛为阶段状态记录 | 已接受 |
| ADR-004 | 2026-08-30 | FYH/docs 沿用团队文档的证据驱动结构风格 | 已接受 |
| ADR-005 | 2026-08-30 | 暂停维护 Scan-Insertion 个人仓库 | 已接受 |
| ADR-006 | 2026-08-30 | FYH 建立独立 Git 历史与 GitHub 远程 | 已接受 |
| ADR-012 | 2026-08-31 | 采用三层任务体系管理 Scan Insertion 项目 | 已接受 |
| ADR-013 | 2026-08-31 | 以 Obsidian 作为个人 Markdown 阅读入口 | 已接受 |
| ADR-014 | 2026-08-31 | 采用具体行动与验收标准的文档表达 | 已接受 |

## ADR-001 — 统一个人记录位置并使用仓库级本地覆盖规则

**日期**：2026-08-30  
**状态**：已接受

### 背景

FYH 同时在团队仓库 `D:\research\2026-NCTIEDA-Semitronix` 和个人
`D:\research\Scan-Insertion` 仓库中工作。若只在 `FYH/` 目录下约束记录，
修改团队仓库其他路径时容易漏记；若在团队规则文件中加入个人要求，又会
污染或改变团队共享政策。

### 决策

以团队仓库中的 `FYH/progress.md` 和 `FYH/decisions.md` 作为跨仓库的个人
权威日志；在团队仓库根目录使用本地、未跟踪的 `AGENTS.override.md`，将
“任务完成前更新 progress、必要时更新 decisions”的要求扩展到整个团队仓库。
`FYH/AGENTS.md` 保留详细记录格式，并明确团队 `AGENTS.md` 与 Git 规则优先。

### 备选方案

- 只依赖 `FYH/AGENTS.md`：无法稳定覆盖 `FYH/` 之外的团队仓库工作。
- 修改团队根 `AGENTS.md`：会把个人流程混入共享规则，增加团队协作负担。
- 在两个仓库各维护一套日志：产生重复来源，难以保持一致。

### 理由

集中日志保持单一事实来源；本地 override 提供工作区范围的提醒，同时通过
Git 忽略规则避免个人偏好进入团队提交。个人仓库现有 `AGENTS.md` 已声明
使用这两个 canonical logs，因此无需复制日志文件。

### 影响

- 每个有意义任务都必须在结束前向 `progress.md` 追加真实记录。
- 只有影响架构、DFT 规则、工具、接口、Schema、工作流或长期取舍的决定才
  追加 ADR。
- `AGENTS.override.md` 只在本地生效，换机器或清理未跟踪文件后需重新建立。

### 受影响组件

- `FYH/AGENTS.md`
- `FYH/progress.md`
- `FYH/decisions.md`
- 团队仓库根 `AGENTS.override.md`
- `.gitignore` 中现有的 `FYH` 与 `AGENTS.override.md` 忽略规则
- `D:\research\Scan-Insertion\AGENTS.md` 的跨仓库记录约定

## ADR-002 — 个人进度与决策日志统一使用中文

**日期**：2026-08-30
**状态**：Accepted

### 背景

FYH 的个人工作记录需要长期回顾，并服务于中文团队协作；此前规则没有
明确日志正文的语言。

### 决策

`FYH/progress.md` 与 `FYH/decisions.md` 的说明性文字统一使用中文。文件
路径、命令、Schema 名称、标识符等技术字面量可保留原文，以确保可执行性
和证据引用准确。

### 备选方案

- 不规定语言：后续条目可能中英文混杂，降低检索和回顾一致性。
- 全部翻译技术字面量：可能破坏命令、路径和接口名称的准确性。

### 理由

中文符合个人与团队的主要工作语境；保留技术字面量可避免记录失真。

### 影响

- 后续新增日志正文必须使用中文。
- 历史条目不强制回写或翻译。

### 受影响组件

- `FYH/AGENTS.md`
- `FYH/progress.md`
- `FYH/decisions.md`
- 团队仓库根 `AGENTS.override.md`

## ADR-003 — 将 progress 从操作流水账收敛为阶段状态记录

**日期**：2026-08-30  
**状态**：Accepted

### 背景

原规则要求每个有意义任务都更新 `progress.md`，导致单个文件修改、普通
调试和测试也可能产生记录，降低了日志对项目阶段判断和上下文恢复的价值。

### 决策

`progress.md` 只在阶段性项目状态变化时更新：稳定可用、首次可用能力、重要
实验结论、milestone、重要 blocker 变化、明显技术路线变化、长时间工作后的
checkpoint，或用户明确要求记录。同一目标的多个修改合并为一条；使用“几天后
重新打开项目是否能快速知道做到哪了？”作为最终判断。任务结束时分别判断
是否形成阶段性状态变化，以及是否形成重要长期技术决策。

### 备选方案

- 保持每个有意义任务必记：信息密度低，难以区分项目状态与操作细节。
- 只在 milestone 记录：可能遗漏重要 blocker、首次可用能力或恢复 checkpoint。

### 理由

阶段状态记录能保留真正影响项目进展的信息，同时覆盖 milestone 之外的关键
转折点，并减少重复和噪声。

### 影响

- 普通 bugfix、临时调试、测试、refactor 和格式修改默认不写入 `progress.md`。
- 既有历史条目不回溯删除；新规则从本 ADR 起生效。
- `decisions.md` 仍只记录重要、长期技术决策，普通实现选择不记录。

### 受影响组件

- `FYH/AGENTS.md`
- `FYH/progress.md`
- `FYH/decisions.md`
- 团队仓库根 `AGENTS.override.md`
- `D:\research\Scan-Insertion\AGENTS.md` 的跨仓库记录段落

## ADR-004 — FYH/docs 沿用团队文档的证据驱动结构风格

**日期**：2026-08-30  
**状态**：Accepted

### 背景

团队 `docs/比赛入门文档` 与 `docs/Agent 与系统预研` 已形成稳定的中文技术
文档风格：结论先行、编号层级、元数据、来源区分、结构化表格、流程图和明确
验收标准。FYH 后续需要在独立目录持续产出文档。

### 决策

将上述风格作为 `FYH/docs` 默认约定：中文说明配合 YAML front matter；按真实
阅读顺序组织编号标题；用表格、代码块和 Mermaid 表达结构；区分正式规则、
宣讲/答疑、实验观察、团队建议和待确认事项；结尾提供验证标准、状态或下一步，
并在文末加入 `## 参考资料`（或等价章节）及真实可点击的引用链接。该约定不
覆盖官方或工具生成文件的既定格式；无外部来源时明确声明，不编造链接。

### 备选方案

- 每篇 FYH 文档自由选择格式：短期灵活，但长期检索和协作一致性较差。
- 直接复制某一篇团队文档的固定模板：无法适配实验记录、方案说明等不同文体。

### 理由

抽取团队文档中跨主题稳定有效的结构原则，同时保留按文档类型裁剪章节的
空间，能兼顾一致性、证据可回查性与实际可读性。

### 影响

- 新建或大幅修改 `FYH/docs` 文档时默认遵循该风格。
- 每次写文档都要在文末保留来源链接，便于复核和恢复上下文。
- 历史 FYH 文档不因风格约定而强制重写。
- 技术字面量保留原文，避免路径、命令和接口引用失真。

### 受影响组件

- `FYH/AGENTS.md`
- `FYH/docs/`（后续文档）
- `FYH/progress.md`
- `FYH/decisions.md`

## ADR-005 — 暂停维护 Scan-Insertion 个人仓库

**日期**：2026-08-30  
**状态**：Accepted

### 背景

FYH 同时曾关联团队仓库和 `D:\research\Scan-Insertion` 个人仓库；当前用户
明确表示后者项目不再进行，继续例行检查会扩大任务范围并产生无效维护成本。

### 决策

将 `D:\research\Scan-Insertion` 标记为暂停维护范围。默认不读取、不修改、
不验证该仓库，也不为其新增进度记录；只有用户明确恢复项目或要求引用特定
资料时才重新启用检查。

### 备选方案

- 继续同步维护两个仓库：与当前项目范围不符，增加噪声和维护成本。
- 删除或归档仓库：超出本次请求范围，也可能破坏历史资料。

### 理由

保留仓库及其历史文件，同时停止无必要的主动操作，既尊重当前项目取舍，也
避免不可逆的删除行为。

### 影响

- 后续团队仓库任务不再默认检查 Scan-Insertion。
- 若未来恢复，需要用户明确提出，并重新确认其规则和工作范围。

### 受影响组件

- `FYH/AGENTS.md`
- `FYH/progress.md`
- `FYH/decisions.md`
- `D:\research\Scan-Insertion`（仅标记为暂停维护，不修改其内容）

## ADR-006 — FYH 建立独立 Git 历史与 GitHub 远程

**日期**：2026-08-30  
**状态**：Accepted

### 背景

FYH 物理上位于团队仓库 `D:\research\2026-NCTIEDA-Semitronix\FYH`，但个人
工作需要独立的提交、分支和远程同步边界。父仓库已忽略 `FYH`，仍需把该目录
正式发布到个人 GitHub 仓库。

### 决策

在 `FYH` 内保留独立 `.git`，以 `master` 作为首个远程分支，并将 `origin`
连接到 GitHub 仓库 `https://github.com/kiddd666/2026-NCTIEDA-FYH`；当前因
HTTPS 出站连接不稳定，实际使用 SSH transport
`git@github.com:kiddd666/2026-NCTIEDA-FYH.git`。FYH 的提交和 push 只使用
FYH 仓库根目录执行；父仓库不纳入 FYH 文件。

### 备选方案

- 将 FYH 作为父仓库普通子目录提交：会混合团队与个人历史，边界不清。
- 使用 Git submodule：需要父仓库提交 submodule 指针，增加团队协作耦合，当前无此需求。
- 仅保留本地仓库、不配置远程：无法完成个人跨环境同步与备份。

### 理由

独立仓库满足个人历史可追溯和 GitHub 备份需求，同时维持父仓库现有忽略规则，
不改变团队仓库的提交模型。

### 影响

- 后续 FYH 变更需在 FYH 仓库单独提交和 push。
- 需要在发布前分别检查两个仓库的根目录、状态和历史，防止误操作。
- 父仓库不会记录 FYH 的提交；跨仓库关联只能通过路径或文档说明表达。

### 受影响组件

- `FYH/.git/`
- `FYH/README.md`
- `FYH/AGENTS.md`
- `FYH/TODO.md`
- 父仓库的 `FYH` 忽略规则（保持不变）

## ADR-012 — 采用三层任务体系管理 Scan Insertion 项目

**日期**：2026-08-31
**状态**：已接受

### 背景

现有文档分别描述赛题规则、团队分工、技术架构和 DFT 学习内容，但缺少从最终
提交倒推的统一任务分解，容易把长期规划复制进 TODO，或把阅读行为与可验收项目
产物混在一起。

### 决策

采用三层任务体系：

1. `00_总项目任务书.md` 定义最终完成边界、模块、硬约束和交付物；
2. `01_阶段任务目标_20260831-20261107.md` 按 6 个阶段和日期定义目标、接口、
   产物、Definition of Done 与进入条件；
3. `02_周行动任务.md` 只展开当前周和紧接下一周的可执行任务，`TODO.md` 仅保留
   当前需要执行的少量任务并以产出为中心。

### 备选方案

- 只维护一份总计划：长期目标与当天行动会互相污染，难以持续更新；
- 把全部阶段任务复制到 TODO：待办入口失去“当前待办”功能；
- 只维护 DFT 学习路线：无法覆盖 Agent、工具、Docker、审计和最终提交。

### 理由

三层分别承担“完成定义—阶段承诺—近期执行”职责，复用现有术语和团队分工；
阶段出口使用可检查产物而不是“学习/开发/测试”等模糊活动，能支持每周复盘和
截止日前的提交门禁。

### 影响

- 2026-08-31～2026-11-07 的阶段目标固定在阶段文档中；
- `TODO.md` 不再承载两个月规划，只保留 FYH-001～FYH-010 的近期任务；
- 官方工具手册、Public case、Liberty、镜像和最新通知未到位的事项必须保持“待确认”，不能凭经验补全；
- 后续阶段变更应先更新阶段文档，再提取当前任务到 TODO。

### 受影响组件

- `docs/00_总项目任务书.md`
- `docs/01_阶段任务目标_20260831-20261107.md`
- `docs/02_周行动任务.md`
- `TODO.md`
- `docs/比赛入门文档/05-待确认事项与来源索引.md`

## ADR-013 — 以 Obsidian 作为个人 Markdown 阅读入口

**日期**：2026-08-31
**状态**：已接受

### 背景

个人项目文档、任务清单和两类记录均以 Markdown 保存；此前引入的额外可视化工具
增加了配置、插件和维护路径，与当前以 Obsidian 阅读和编辑文档的习惯不一致。

### 决策

撤销额外卡片工具、VS Code 工作区配置、本地 Codex skill、同步脚本和卡片目录。
今后 `TODO.md`、`docs/`、`progress.md` 与 `decisions.md` 均作为普通 Markdown
文件维护，并以 Obsidian 作为个人阅读入口。

### 备选方案

- 保留额外可视化工具：需要继续维护配置和数据同步，收益与当前需求不匹配。
- 采用常驻文件监听器：增加后台进程和故障排查成本，不利于个人仓库的可预测性。

### 理由

普通 Markdown 在 Obsidian、Git 和命令行中均可直接使用，文件结构简单，减少工具
耦合，并保持团队文档风格和个人记录规则的一致性。

### 影响

- 新增或修改任务只维护 `TODO.md`，不再生成额外卡片文件。
- VS Code 根工作区保留与任务清单相关的既有设置，不添加额外插件配置。
- 后续文档继续遵守 YAML front matter、结构化章节和文末引用链接约定。

### 受影响组件

- `TODO.md`
- `AGENTS.md`
- `README.md`
- `docs/02_周行动任务.md`
- `progress.md`
- `decisions.md`

## ADR-014 — 采用具体行动与验收标准的文档表达

**日期**：2026-08-31
**状态**：已接受

### 背景

个人文档需要在几天后重新打开时仍然容易阅读和执行。过多的抽象词、口号式总结
和固定模板会让读者难以判断谁要做什么，也不容易找到产出和完成条件。

### 决策

FYH 文档沿用团队《04-团队角色与近期行动》的写法：先说明分工和共同原则，再
列出近期行动、产出、依赖、完成标准和待确认问题。正文优先使用短句和具体动词，
少用抽象词和口号式表达；需要概括时，改写为具体动作、负责人和可检查结果。

### 备选方案

- 继续使用统一但抽象的模板：结构看似整齐，阅读时仍需自行解释行动含义。
- 只写连续叙述：上下文自然，但任务、负责人和验收条件容易遗漏。

### 理由

团队参考文档已经验证了“分工—原则—行动—标准—待确认”的组织方式，读者可以
直接从表格和清单定位责任、产出和阻塞点；具体动词也更适合后续执行和复核。

### 影响

- 新建或大幅修改的 FYH 文档应优先采用上述结构，按内容需要取舍章节。
- 表格用于职责、任务、产出、依赖和验收标准；不为形式强行添加图表或固定段落。
- 现有文档不因词语偏好而全面重写，只有在后续任务涉及相关内容时顺手修正。

### 受影响组件

- `AGENTS.md`
- `docs/`
- `progress.md`
- `decisions.md`

## ADR-015 — 在 FYH 内维护独立文件树

**日期**：2026-09-02
**状态**：已接受

### 背景

FYH 是嵌套在团队仓库中的独立个人仓库。随着文档、提示词和技术路线资料增多，
仅依靠目录浏览不便于 Agent 快速定位文件；父仓库的文件树又不应收录私人内容。

### 决策

在 FYH 内复制并部署 `file-tree` 技能，以
`.agents/skills/file-tree/tree.json` 作为唯一数据源，并将简版树渲染到 FYH
自己的 `AGENTS.md`。文件树的增删改移统一通过 `tree_tool.py` 完成。

### 备选方案

- 继续复用父仓库文件树：会混淆两个仓库的边界，且无法准确反映 FYH 内容。
- 手写一份静态目录树：维护成本高，容易与磁盘和 Git 状态不一致。

### 理由

独立部署保持私人仓库边界，同时提供可查询的结构化数据和 Agent 入口；脚本化维护
可通过 `check --strict` 验证树、磁盘与渲染产物的一致性。

### 影响

- FYH 新增、删除或移动文件时需同步运行文件树脚本；
- 父仓库不收录 FYH 文件树及其他私人文件；
- `AGENTS.md` 增加文件树使用说明和自动渲染区块。

### 受影响组件

- `.agents/skills/file-tree/`
- `AGENTS.md`
- `progress.md`

## 参考资料

- [04-团队角色与近期行动](../docs/比赛入门文档/04-团队角色与近期行动.md)

## ADR-016 — 先用开源前置闭环替换工具专用学习

**日期**：2026-09-02
**状态**：已接受

### 背景

当前尚未拿到广立微 Scan Insertion 工具手册和 Public Case，但已经具备基础
Verilog、HDLBits、DFT Chapter 1/2.1～2.7 以及少量 Scan 实验。若继续等待工具材料，
网表分析、Scan 验证、时钟风险和证据链能力不会增长；若凭经验猜测广立微命令，
又会把未经证实的规则带入比赛实现。

### 决策

未来四天先建立一个不依赖广立微私有命令的开源前置闭环：基线 Scan 回归 →
网表/Liberty 联合分析 → 手工 Scan 与 Lockup → EQY 三态 → 模拟 case 的
`found → diagnosis → fix → verify`。工具手册和 Public Case 到位后，只替换
工具执行与报告适配层，保留需求原子化、证据引用、失败保留和结果断言。

### 备选方案

- 继续按教材章节顺序阅读：理论覆盖更广，但短期缺少可迁移的运行证据。
- 现在猜写广立微 Dofile：表面接近比赛流程，但违反证据优先原则，风险不可接受。
- 直接等待 Public Case：避免返工，但无法利用当前时间建立实验习惯和验证器。

### 理由

开源工具可以真实验证通用结构和流程能力，同时不把 Yosys/Fault/OpenSTA 的行为
冒充广立微语义。约 70% 实验投入也与当前最主要缺口（GAP-03、GAP-04、GAP-07）
一致，且能为后续 Dofile/报告适配提供输入、输出和验收模型。

### 影响

- 四天计划以实验产物和自动化 PASS/FAIL 为主要完成证据；
- Fault、OpenSTA 均为可选增强，不得阻塞基线 Scan、EQY 和模拟 case 主线；
- 广立微命令、DFTR 编号、报告字段在手册到位前只记录为待确认事项；
- 未来在 `FYH/experiments` 中配置独立实验包，不修改已暂停的 `D:\research\Scan-Insertion`。

### 受影响组件

- `docs/技术路线与实验规划/06/_9.2学习计划与实验方案.md`
- `FYH/experiments/`
- 后续 `requirement_matrix`、`report_assertions`、`dofile_engine` 与 `eqy` 实验资产
